class Snake {
    ...
    constructor(size: number, canvasWidth: number, canvasHeight: number) {
    ...
    update(event: KeyboardEvent | null = null) {

        const updateDirection = (event: KeyboardEvent): void => {
            switch (event.key) {
                case 'ArrowUp':
                    this.direction = 'up';
                    break;
    ...
    collidesWith(x: number, y: number, size: number) {
        const head = this.segments[this.segments.length - 1];
        return (
            head.x < x + size &&
            head.x + this.size > x &&
    ...
    reset(size: number, canvasWidth: number, canvasHeight: number) {
        this.segments = [{x: 0, y: 0}];
        this.size = size;
        this.sessionScore = 0;
        this.direction = 'left';
